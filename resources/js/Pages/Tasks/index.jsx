                ...
                <div className="w-full p-4 text-center bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                    <div className="pb-4 flex flex-wrap">
                        <div className="flex mx-1 my-px">
                            <Dropdown>
                                <Dropdown.Trigger>
                                    <button className="px-4 py-2 bg-blue-500 text-white rounded-md">Select Columns</button>
                                </Dropdown.Trigger>
                                    ...
                                    {Object.keys(visibleColumns).map((column) => (
                                        <div key={column} className="px-4">
                                            <div className="flex items-center mb-2">
                                                ...
                                                />
                                                <span className="ml-2"> {column.replace('_', ' ').toUpperCase()} </span>
                                            </div>
                                        </div>
                                    ))}
                                </Dropdown.Content>
                                ...
                                <tbody>
                                    {tasks.data.map((task) => (
                                        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={task.id}>
                                            {visibleColumns.id && <td className="text-center">{task.id}</td>}
                                            {visibleColumns.name && <td>{task.name}</td>}
                                            {visibleColumns.description && <td>{task.description}</td>}
                                            {visibleColumns.status && (
                                                <td>
                                            ...
                                            {visibleColumns.image && <td><img className="rounded-full w-20 h-20" src={task.image_path} /></td>}
                                            {visibleColumns.created_at && <td>{task.created_at}</td>}
                                            {visibleColumns.due_date && <td>{task.due_date}</td>}
                                            {visibleColumns.created_by && <td>{task.created_by.name}</td>}
                                            {visibleColumns.updated_by && <td>{task.updated_by.name}</td>}
                                            <td>
